...
舞台实物由 `createModelSlot(ctx, { id, label, glyph, hint })`（`src/ui/model-slot.js`）创建，`slot.set(state)` 传递玩法状态。已内置罗盘、星仪、水晶球、灵摆和四柱命牌；`window.__lgj.registerModel(id, factory)` 可覆盖同名模型。模块不读取模型内部 DOM。状态契约见 `docs/MODEL_SLOTS.md`，新增模型请更新对应条目。
